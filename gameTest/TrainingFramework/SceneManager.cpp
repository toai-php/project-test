...
void SceneManager::Init() {
	...
	printf("%s\n", m_fileSM);
	FILE* f_SM;
	f_SM = fopen("../Resources/Managers/SM1.txt", "r+");
	if (f_SM != NULL) {
		this->ReadFile(f_SM);
	}
	FILE* f_MAP;
	f_MAP = fopen("../Resources/Map/maplv1.txt", "r+");
	if (f_MAP != NULL) {
		this->ReadMap(f_MAP);
	}
	...
	for (register int i = 0; i < numOfObjects; i++) {
		...
		if (num_anim > 0) {
			...
			for (int i = 0; i < num_anim; i++) {
				...
				pModel->addAnimation(ResourcesManager::getInstance()->GetAnimationAtID(anim));
			}
		}
		...
		if (strcmp(type, "MAIN") == 0) {
			...
			m_MainCharacter->setShader(ResourcesManager::getInstance()->GetShaderAtID(shader));
			m_MainCharacter->SetTexture(ResourcesManager::getInstance()->GetTextureAtID(texture));
			...
			enemy->setShader(ResourcesManager::getInstance()->GetShaderAtID(shader));
			enemy->SetTexture(ResourcesManager::getInstance()->GetTextureAtID(texture));
			...
			m_boss->setShader(ResourcesManager::getInstance()->GetShaderAtID(shader));
			m_boss->SetTexture(ResourcesManager::getInstance()->GetTextureAtID(texture));
			...
			bullet->setShader(ResourcesManager::getInstance()->GetShaderAtID(shader));
			bullet->SetTexture(ResourcesManager::getInstance()->GetTextureAtID(texture));
...
void SceneManager::ReadMap(FILE *f_MAP) {
	...
	groundTest->setShader(ResourcesManager::getInstance()->GetShaderAtID(0));
	//
	...
	for (int i = 0; i < num; i++) {
		...
		background->setShader(ResourcesManager::getInstance()->GetShaderAtID(0));
		background->SetTexture(ResourcesManager::getInstance()->GetBackgroundAtID(index));
...
void SceneManager::Draw() {
	...
	glUseProgram(ResourcesManager::getInstance()->GetShaderAtID(0)->program);
